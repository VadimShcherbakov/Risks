...
class WriteNewRisk:
    ...
    def check_input(self, key_value, check_value):
        """Проверяет не пустой пользовательский ввод"""
        if len(check_value) > 0 and check_value[-1]:
            return check_value
        messagebox.showerror(LEXICON_RU['error'], f"заполните поле {LEXICON_RU[key_value]}")
    ...
    def create_another(self, num_classificator: str, division: str, before_severity_assessmentinput: int,
        ...
        def get_all_date():
            ...
            Risk.create(
                ...
                clas_dangerous=num_classificator.split()[0],
        ...
        tk.Label(frame, text=LEXICON_RU['change_rows']).grid(row=7, column=0, sticky='w')
        combo_change_rows = ttk.Combobox(frame, values=(dict_division['rows']))
        combo_change_rows.grid(row=7, column=1, sticky='w')
        # ----------Создаём блок выбора осей----------
        tk.Label(frame, text=LEXICON_RU['change_axes']).grid(row=8, column=0, sticky='w')
        combo_change_axes = ttk.Combobox(frame, values=[i for i in range(dict_division['axes'][0], dict_division['axes'][1]+1)] if dict_division['axes'] else [])
        combo_change_axes.grid(row=8, column=1, sticky='w')
        # ----------Создаём блок описания места риска----------
        tk.Label(frame, text=LEXICON_RU['region']).grid(row=9, column=0, sticky='w')
        entry_region = ttk.Entry(width=50)
        entry_region.grid(row=9, column=1, sticky='w')
        # ----------Создаём блок выбора отметки----------
        tk.Label(frame, text=LEXICON_RU['level']).grid(row=10, column=0, sticky='w')
        ...
        tk.Label(frame, text=LEXICON_RU['description']).grid(row=11, column=0, sticky='w')
        entry_description = ttk.Entry(width=50)
        entry_description.grid(row=11, column=1, sticky='w')
        # ----------Создаём блок выбора владельца риска----------
        tk.Label(frame, text=LEXICON_RU['change_owner']).grid(row=12, column=0, sticky='w')
        combo_change_owner_risk = ttk.Combobox(frame, values=dict_division['owner_risk'])
        combo_change_owner_risk.grid(row=12, column=1, sticky='w')
        combo_change_owner_risk.current(0)

        tk.Label(frame, text=LEXICON_RU['event_first']).grid(row=13, column=0, columnspan=3)

        # ----------Создаём блок мероприятия по устранению риска----------
        tk.Label(frame, text=LEXICON_RU['elimination_measures']).grid(row=14, column=0, sticky='w')
        ...
        tk.Label(frame, text=LEXICON_RU['responsible_for_implementation']).grid(row=15, column=0, sticky='w')
        combo_responsible_for_implementation_1 = ttk.Combobox(frame, values=dict_division["managers"])
        combo_responsible_for_implementation_1.grid(row=15, column=1, sticky='w')
        # ----------Создаём блок выбора срока устранения----------
        tk.Label(frame, text=LEXICON_RU['period_of_execution']).grid(row=16, column=0, sticky='w')
        combo_period_of_execution_1 = ttk.Combobox(frame, values=[0, 1, 2, 4, 8, 26, 52])
        combo_period_of_execution_1.grid(row=16, column=1, sticky='w')
        # ----------Создаём блок для написания ожидаемого результата----------
        tk.Label(frame, text=LEXICON_RU['expected_result']).grid(row=17, column=0, sticky='w')
        ...
        if before_grade >= 6:
            tk.Label(frame, text=LEXICON_RU['event_second']).grid(row=18, column=0, columnspan=3)
            # ----------Создаём блок мероприятия по устранению риска----------
            tk.Label(frame, text=LEXICON_RU['elimination_measures']).grid(row=19, column=0, sticky='w')
            entry_elimination_measures_2 = ttk.Entry(width=50)
            entry_elimination_measures_2.grid(row=19, column=1, sticky='w')
            # ----------Создаём блок выбора ответственного за устранение----------
            tk.Label(frame, text=LEXICON_RU['responsible_for_implementation']).grid(row=20, column=0,
                                                                                                 sticky='w')
            combo_responsible_for_implementation_2 = ttk.Combobox(frame, values=dict_division["managers"])
            combo_responsible_for_implementation_2.grid(row=20, column=1, sticky='w')
            # ----------Создаём блок выбора срока устранения----------
            tk.Label(frame, text=LEXICON_RU['period_of_execution']).grid(row=21, column=0, sticky='w')
            combo_period_of_execution_2 = ttk.Combobox(frame, values=[0, 1, 2, 4, 8, 26, 52])
            combo_period_of_execution_2.grid(row=21, column=1, sticky='w')
            # ----------Создаём блок для написания ожидаемого результата----------
            tk.Label(frame, text=LEXICON_RU['expected_result']).grid(row=22, column=0, sticky='w')
            entry_expected_result_2 = ttk.Entry(width=50)
            entry_expected_result_2.grid(row=22, column=1, sticky='w')
            # ----------Создаём кнопку заполнить----------
            button_fin = tk.Button(frame, text=LEXICON_RU['fin_button'], command=get_all_date)
    ...
    def create_division(self, classificator: str) -> None:
        ...
        tk.Label(frame, text=LEXICON_RU['change_num_type_dangerous']).grid(row=3, column=0, sticky='w')
        combo_change_class_risk = ttk.Combobox(frame, width=95,
                                               values=class_dangerous[classificator.split()[0]])
        combo_change_class_risk.grid(row=3, column=1, columnspan=2, sticky='w')
        # ----------Создаём блок выбора оценки тяжести----------
        tk.Label(frame, text=LEXICON_RU['before_severity_assessmentinput']).grid(row=4, column=0,
                                                                                              sticky='w')
        combo_change_before_severity_assessmentinput = ttk.Combobox(frame, values=[i for i in range(2, 6)])
        ...
        tk.Label(frame, text=LEXICON_RU['before_probability_estimation']).grid(row=5, column=0,
                                                                                            sticky='w')
        combo_change_before_probability_estimation = ttk.Combobox(frame, values=[i for i in range(2, 6)])
        ...
        tk.Label(frame, text=LEXICON_RU['change_division']).grid(row=6, column=0, sticky='w')
        combo_change_division = ttk.Combobox(frame, values=(guide['name_division']))
        combo_change_division.grid(row=6, column=1, sticky='w')
        button_change_division = tk.Button(frame, text=LEXICON_RU['select_button'],
                                           command=lambda: self.create_another(self.check_input("change_num_type_dangerous",combo_change_class_risk.get()),
                                                                               self.check_input("change_division", combo_change_division.get()),
    ...
    def change_dangerous(self):
        ...
        tk.Label(frame, text=LEXICON_RU['num_clas_dangerous']).grid(row=1, column=0, sticky='w')
        combo_change_type_risk = ttk.Combobox(frame, width=50, values=class_dangerous["class_dangerous"])
        combo_change_type_risk.grid(row=1, column=1, sticky='w')
        button_change_dangerous = tk.Button(frame, text=LEXICON_RU['select_button'],
                                            command=lambda: self.create_division(
                                                self.check_input("num_clas_dangerous", combo_change_type_risk.get())))
...
if __name__ == '__main__':
    for dirpath, subdirs, files in os.walk(photo_raw_risk_before):
        for raw_photo_name in files:
            ...
            for r in range(21): frame.rowconfigure(index=r, weight=1)
            WriteNewRisk().start()
            frame.mainloop()
